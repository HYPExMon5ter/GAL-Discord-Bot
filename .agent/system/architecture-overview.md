...
- ✅ **Google Sheets**: Registration data, smart caching (10min refresh)
- ✅ **Riot API**: Player verification and statistics
- ✅ **IGN Verification**: IGN validation system
- ❌ **Live Graphics Dashboard**: **Dashboard temporarily deprecated — to be reintroduced post-DB migration.**

### Data Flow
```
Discord Commands → Core Logic → External APIs → Database → Discord UI
     ↓                ↓              ↓           ↓         ↓
Slash Commands → Business Logic → Sheets/Riot → Cache → Embeds/Views
```
