...
- **Status (2025-10-16):** Routers now consume injected services via shared dependencies, service layer raises structured errors, WebSocket endpoint enforces backpressure with bounded queues, auth tests cover master password flows, deprecated datetime/pydantic/sqlalchemy touchpoints are cleared, and `python -m pytest` (API suite) completes cleanly with no warnings.
