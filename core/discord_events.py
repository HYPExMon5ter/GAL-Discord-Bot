...
def register_discord_events(bot: commands.Bot):
    """Register all Discord event handlers for the bot."""

    if not hasattr(bot, "_discord_events_ready"):
        bot._discord_events_ready = False
    ...
    async def on_ready():
        ...
        if getattr(bot, "_discord_events_ready", False):
            logging.debug("Discord events already initialized; skipping on_ready setup")
            return
        ...
        try:
            ...
            for guild in bot.guilds:
                try:
                    ...
                    try:
                        ...
                        await asyncio.wait_for(refresh_sheet_cache(bot=bot, force=True), timeout=30.0)
                        logging.info(f"Cache refreshed for guild {guild.name} ({guild.id})")
                    except asyncio.TimeoutError:
                        logging.error(f"Cache refresh timed out for guild {guild.name} ({guild.id}) - continuing startup")
                    except Exception as e:
                        logging.error(f"Failed to refresh cache for guild {guild.name} ({guild.id}): {e} - continuing startup")
            ...
            bot._discord_events_ready = True
    ...
    async def on_disconnect():
        """Reset discord event readiness when the bot disconnects."""
        bot._discord_events_ready = False
