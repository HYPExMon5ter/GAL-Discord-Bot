...
content = """# Screenshot Extraction System - Testing Guide
...
### Upload Screenshot
```bash
curl -X POST http://localhost:8000/api/v1/placements/process-screenshot \
...
### Approve
```bash
curl -X POST http://localhost:8000/api/v1/placements/validate/5 \
  -d '{"approved": true}'
```
...
```bash
curl -X POST http://localhost:8000/api/v1/placements/validate/5 \
  -d '{"approved": true, "edited_placements": [...]}'
```
...
with open(r'C:\Users\blake\PycharmProjects\New-GAL-Discord-Bot\SCREENSHOT_SYSTEM_TESTING_GUIDE.md', 'w', encoding='utf-8') as f:
    f.write(content)

print('Testing guide created successfully!')
